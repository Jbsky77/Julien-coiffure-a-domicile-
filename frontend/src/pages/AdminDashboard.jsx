...
const STATUS_LABELS = {
  ...
  unpaid: "Impayé", canceled: "Résilié", suspended: "Suspendu", incomplete: "À configurer",
};
const STATUS_STYLES = {
...
const PERIODS = [{ id: "30d", label: "30 jours" }, { id: "90d", label: "90 jours" }, { id: "year", label: "Cette année" }, { id: "all", label: "Depuis le début" }];
const TABS = [{ id: "activity", label: "Activité", icon: TrendingUp }, { id: "map", label: "Carte", icon: MapPin }, { id: "companies", label: "Entreprises", icon: Building2 }];
const COLORS = ["#0A192F", "#D4AF37", "#2563EB", "#059669", "#7C3AED", "#EA580C"];
const euro = (value) => `${Number(value || 0).toLocaleString("fr-FR", { maximumFractionDigits: 2 })} €`;
    ...
    <article className="bg-white border border-slate-100 rounded-3xl p-5 md:p-6 shadow-sm" data-testid={`admin-company-${company.id}`}>
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-x-5 gap-y-1 text-xs text-slate-500 mt-3"><span>SIRET : {company.siret || "À compléter"}</span><span>{company.user_count} utilisateur{company.user_count > 1 ? "s" : ""}</span><span>{company.employee_count} employé{company.employee_count > 1 ? "s" : ""}</span><span>Inscrite le {new Date(company.created_at).toLocaleDateString("fr-FR")}</span></div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mt-5">
        ...
        <button type="button" disabled={!changed || saving} onClick={() => onSave(company, form)} className="flex-1 border border-slate-200 rounded-full px-5 py-3 font-medium disabled:opacity-40">{saving ? "Enregistrement…" : "Enregistrer l’abonnement"}</button>
      </div>
    </article>
  );
}
  ...
  return (
    <div className="space-y-6">
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        <Metric icon={Euro} label="Chiffre d’affaires" value={euro(totals.revenue)} tone="gold" />
        <Metric icon={CalendarDays} label="Rendez-vous" value={totals.appointments || 0} hint={`${totals.completed || 0} terminés`} />
        <Metric icon={Users} label="Clients" value={totals.clients_total || 0} hint={`${totals.new_clients || 0} nouveaux`} tone="blue" />
        <Metric icon={Scissors} label="Employés" value={totals.employees || 0} tone="green" />
        <Metric icon={WalletCards} label="Panier moyen" value={euro(totals.average_basket)} tone="gold" />
        <Metric icon={TrendingUp} label="Taux d’annulation" value={`${totals.cancellation_rate || 0} %`} />
      </section>
      <section className="grid lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 bg-white rounded-3xl border border-slate-100 p-5 shadow-sm">
          <div className="text-[10px] uppercase tracking-[0.28em] text-slate-500 mb-4">Évolution de l’activité</div>
...
function AdminMap({ data }) {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-2xl border border-slate-100 p-4 flex flex-wrap items-center gap-4"><div><div className="text-2xl font-semibold">{data.map?.located_clients || 0}</div><div className="text-xs uppercase tracking-wider text-slate-500">Clients localisés</div></div><div className="h-10 w-px bg-slate-100"/><div className="flex flex-wrap gap-2">{companies.map((id) => { const p = points.find((item) => item.company_id === id); return <span key={id} className="inline-flex items-center gap-2 text-xs px-3 py-1.5 rounded-full bg-slate-50"><span className="w-2.5 h-2.5 rounded-full" style={{ background: colors[id] }}/>{p?.company_name}</span>; })}</div></div>
      ...
      <div className="text-xs text-slate-400">La carte affiche uniquement les clients dont les coordonnées géographiques sont disponibles. Les couleurs distinguent les entreprises sélectionnées.</div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-slate-50" data-testid="platform-admin-dashboard">
      ...
      <main className="max-w-7xl mx-auto px-5 md:px-8 py-8">
        ...
        {loading && !overview.companies.length ? <div className="bg-white rounded-2xl p-10 text-center text-slate-500">Chargement du pilotage…</div> : <>{tab === "activity" && <ActivityView data={analytics}/>} {tab === "map" && <AdminMap data={analytics}/>} {tab === "companies" && <div className="space-y-6"><section className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4"><Metric icon={Building2} label="Entreprises" value={stats.companies_total}/><Metric icon={Users} label="Utilisateurs" value={stats.users_total} tone="blue"/><Metric icon={CreditCard} label="Abonnements payants" value={(stats.subscriptions_monthly || 0) + (stats.subscriptions_quarterly || 0) + (stats.subscriptions_annual || 0)} tone="gold"/><Metric icon={ShieldCheck} label="À régulariser" value={stats.companies_blocked} tone="green"/></section><div className="space-y-4">{overview.companies.map((company) => <CompanyCard key={company.id} company={company} onOpen={openCompany} onSave={saveSubscription} saving={savingId === company.id}/>)}</div></div>}</>}
      </main>
    </div>
  );
}
