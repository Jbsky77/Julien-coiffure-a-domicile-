...
export default function Login() {
  ...
  const forgotPassword = async () => {
    if (!email.trim()) {
      toast.error("Saisissez dâ€™abord votre adresse e-mail.");
      return;
    }
    ...
    try {
      ...
      toast.success("Si un compte existe, un e-mail de rÃ©initialisation a Ã©tÃ© envoyÃ©.");
    } catch (error) {
      toast.error(error.message || "Impossible dâ€™envoyer lâ€™e-mail de rÃ©initialisation.");
    } finally {
      setResetting(false);
    }
  };
    ...
    <div className="min-h-screen bg-white flex flex-col md:flex-row">
      <div className="hidden md:flex md:w-1/2 relative overflow-hidden">
        ...
        <div className="relative z-10 p-16 flex flex-col justify-between text-white w-full">
          ...
          <div>
            <div className="text-[11px] tracking-[0.3em] uppercase text-[#D4AF37] mb-4">Votre activitÃ©, simplement</div>
            <h1 className="font-serif text-5xl leading-[1.05] tracking-tight mb-6">Pilotez votre entreprise,<br /><span className="italic text-[#D4AF37]">en toute sÃ©rÃ©nitÃ©.</span></h1>
            <p className="text-white/70 max-w-md text-base leading-relaxed">Une plateforme sÃ©curisÃ©e pour gÃ©rer votre activitÃ©, votre Ã©quipe et vos clients.</p>
          </div>
          ...
          <div className="text-[10px] tracking-[0.3em] uppercase text-white/40">Premium Â· Mobile Â· SÃ©curisÃ©</div>
        </div>
      </div>
      <div className="flex-1 flex items-center justify-center px-6 py-16">
        <form onSubmit={submit} className="w-full max-w-md space-y-6">
          ...
          <div>
            <div className="text-[10px] tracking-[0.3em] uppercase text-slate-500 mb-3">Espace professionnel sÃ©curisÃ©</div>
          ...
          <div>
            ...
            <button type="button" onClick={forgotPassword} disabled={resetting} data-testid="forgot-password-btn" className="mt-3 text-sm text-[#1E3A8A] hover:underline disabled:opacity-50">
              {resetting ? "Envoi en coursâ€¦" : "Mot de passe oubliÃ© ?"}
            </button>
          </div>
