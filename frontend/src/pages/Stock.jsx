...
const label = (product) => [product?.productName, product?.shadeCode, product?.shadeName, product?.format].filter(Boolean).join(" Â· ");

function statusFor(item) {
  const quantity = q(item.quantity);
  const threshold = q(item.reorder_threshold ?? item.threshold ?? 0);
  if (quantity < 0) return { label: "Stock nÃ©gatif â€” Ã  commander", cls: "bg-red-50 border-red-200 text-red-800" };
  if (quantity === 0) return { label: "Rupture â€” Ã  commander", cls: "bg-red-50 border-red-200 text-red-800" };
  if (quantity <= threshold) return { label: "Stock faible â€” Ã  commander", cls: "bg-amber-50 border-amber-200 text-amber-900" };
  return { label: "Disponible", cls: "bg-emerald-50 border-emerald-200 text-emerald-800" };
}

function AddProductDialog({ catalog, onClose, onAdded }) {
  ...
  const submit = async () => {
    if (!selected) return toast.error("SÃ©lectionnez un produit");
    if (!Number.isInteger(Number(form.quantity)) || Number(form.quantity) <= 0) return toast.error("La quantitÃ© doit Ãªtre un entier supÃ©rieur Ã  zÃ©ro");
    if (form.unitPrice !== "" && (Number(form.unitPrice) < 0 || !/^\d+(?:[.,]\d{1,2})?$/.test(String(form.unitPrice)))) return toast.error("Le tarif doit Ãªtre positif avec deux dÃ©cimales maximum");
    setSaving(true);
    try {
      await api.post("/stock/catalog-add", { catalog_product_id: selected.id, quantity: Number(form.quantity), unit_price: form.unitPrice === "" ? null : Number(String(form.unitPrice).replace(",", ".")) });
      toast.success(form.unitPrice === "" ? "QuantitÃ© ajoutÃ©e, tarif existant conservÃ©" : "QuantitÃ© ajoutÃ©e et tarif unitaire mis Ã  jour");
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-3xl max-h-[95vh] overflow-y-auto p-5 md:p-7 space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <label className="text-xs text-slate-600">Marque *
            <select value={form.brand} onChange={(e) => setForm({ ...form, brand: e.target.value, category: "", range: "", productId: "" })} className={selectClass}><option value="">SÃ©lectionnerâ€¦</option>{brands.map((x) => <option key={x}>{x}</option>)}</select>
          </label>
          <label className="text-xs text-slate-600">CatÃ©gorie *
            <select disabled={!form.brand} value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value, range: "", productId: "" })} className={selectClass}><option value="">SÃ©lectionnerâ€¦</option>{categories.map((x) => <option key={x}>{x}</option>)}</select>
          </label>
          <label className="text-xs text-slate-600">Gamme *
            <select disabled={!form.category} value={form.range} onChange={(e) => setForm({ ...form, range: e.target.value, productId: "" })} className={selectClass}><option value="">SÃ©lectionnerâ€¦</option>{ranges.map((x) => <option key={x}>{x}</option>)}</select>
          </label>
        </div>
        ...
        <label className="text-xs text-slate-600 block">Produit *
          <select disabled={!form.range && !form.search} value={form.productId} onChange={(e) => setForm({ ...form, productId: e.target.value })} className={selectClass} size={Math.min(6, Math.max(1, products.length))} data-testid="catalog-product-select">
            {products.map((p) => <option key={p.id} value={p.id}>{p.range} â€” {label(p)}</option>)}
          </select>
        </label>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <label className="text-xs text-slate-600">QuantitÃ© ajoutÃ©e *
          ...
          <label className="text-xs text-slate-600">Tarif unitaire (facultatif)
            <div className="relative"><input inputMode="decimal" value={form.unitPrice} onChange={(e) => setForm({ ...form, unitPrice: e.target.value })} placeholder="Non renseignÃ©" className={`${selectClass} pr-9`} /><span className="absolute right-3 top-4 text-slate-500">â‚¬</span></div>
          </label>
        </div>
        {selected && <div className="rounded-2xl bg-slate-50 border border-slate-100 p-4 text-sm"><strong>RÃ©sumÃ© :</strong> {selected.brand} Â· {selected.normalizedCategory} Â· {selected.range} Â· {label(selected)} Â· quantitÃ© {form.quantity}{form.unitPrice !== "" ? ` Â· ${form.unitPrice} â‚¬ l'unitÃ© (remplace le tarif actuel)` : " Â· tarif actuel conservÃ©"}</div>}
        <div className="flex flex-col-reverse md:flex-row md:justify-end gap-2"><button type="button" onClick={onClose} className="rounded-full border border-slate-200 px-5 py-3">Annuler</button><button type="button" disabled={saving || !selected} onClick={submit} className="rounded-full bg-[#0A192F] text-white px-6 py-3 disabled:opacity-50">{saving ? "Ajoutâ€¦" : "Ajouter au stock"}</button></div>
      </div>
    </div>
  );
}
...
function StockLine({ item, showPrices, onChanged }) {
  ...
  const save = async () => {
    setSaving(true);
    try {
      const payload = mode === "price" ? { mode, unit_price: price === "" ? null : Number(price), remove_price: price === "", reorder_threshold: Number(thresholdEdit), target_stock: Number(targetEdit) } : { mode, quantity: Number(amount), note: "Modification depuis l'Ã©cran Stock", reorder_threshold: Number(thresholdEdit), target_stock: Number(targetEdit) };
      ...
      toast.success("Stock mis Ã  jour"); setEditing(false); await onChanged();
  ...
  const remove = async () => {
    if (!window.confirm(`Retirer dÃ©finitivement Â« ${item.name} Â» du stock ? L'historique des mouvements sera conservÃ©.`)) return;
    await api.delete(`/stock/${item.id}`); toast.success("Produit retirÃ©"); onChanged();
  };
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-4" data-testid={`stock-item-${item.id}`}>
      <div className="flex items-start gap-3">
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-slate-500">{[product.shadeName, product.format].filter(Boolean).join(" Â· ") || item.tag}</div>
          ...
          {q(item.quantity) <= threshold && <div className="text-xs text-red-700 mt-1"><Bell className="inline w-3 h-3 mr-1" />QuantitÃ© recommandÃ©e : commander {recommended.toLocaleString("fr-FR")} {unit}{recommended > 1 ? "s" : ""}</div>}
          {showPrices && <div className="text-xs text-slate-600 mt-2">Tarif : {item.unit_price === null || item.unit_price === undefined ? "Non renseignÃ©" : euro(item.unit_price)} Â· Valeur : {item.unit_price === null || item.unit_price === undefined ? "Non renseignÃ©e" : euro(q(item.quantity) * item.unit_price)}</div>}
        </div>
      ...
      {editing && <div className="mt-4 pt-4 border-t border-slate-100 space-y-3">
        ...
        <button disabled={saving} onClick={save} className="rounded-full bg-[#0A192F] text-white px-4 py-2 text-sm disabled:opacity-50">{saving ? "Enregistrementâ€¦" : "Valider"}</button>
      </div>}
      ...
      {movements && <div className="mt-2 space-y-1.5">{movements.length === 0 ? <div className="text-xs text-slate-400">Aucun mouvement.</div> : movements.map((m) => <div key={m.id} className="rounded-xl bg-slate-50 px-3 py-2 text-xs flex justify-between gap-2"><span>{new Date(m.created_at).toLocaleString("fr-FR")} Â· {m.reason || m.movement_type}</span><strong>{m.quantity_delta > 0 ? "+" : ""}{q(m.quantity_delta).toLocaleString("fr-FR")}</strong></div>)}</div>}
    </article>
  );
}
...
export default function Stock() {
  ...
  const reset = () => { setSearch(""); setBrand(""); setCategory(""); setOrderOnly(false); };
  const addLegacy = async () => {
    if (!legacyForm.name.trim()) return toast.error("Le nom est obligatoire");
    try { await api.post("/stock", { ...legacyForm, name: legacyForm.name.trim(), quantity: Number(legacyForm.quantity), threshold: Number(legacyForm.threshold) }); toast.success("Produit libre ajoutÃ©"); setLegacyForm({ name: "", quantity: 1, threshold: 0, tag: "Autre" }); setShowLegacyAdd(false); load(); }
  ...
  return (
    <div className="space-y-7" data-testid="stock-page">
      ...
      {showLegacyAdd && <div className="bg-white border border-slate-100 rounded-2xl p-5 grid grid-cols-1 md:grid-cols-5 gap-4 shadow-premium"><label className="text-xs md:col-span-2">Nom<input value={legacyForm.name} onChange={(e) => setLegacyForm({ ...legacyForm, name: e.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-3" /></label><label className="text-xs">QuantitÃ©<input type="number" value={legacyForm.quantity} onChange={(e) => setLegacyForm({ ...legacyForm, quantity: e.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-3" /></label><label className="text-xs">Seuil<input type="number" value={legacyForm.threshold} onChange={(e) => setLegacyForm({ ...legacyForm, threshold: e.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-3" /></label><label className="text-xs">CatÃ©gorie<input value={legacyForm.tag} onChange={(e) => setLegacyForm({ ...legacyForm, tag: e.target.value })} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-3" /></label><div className="md:col-span-5 flex justify-end"><button onClick={addLegacy} className="rounded-full bg-[#0A192F] text-white px-5 py-3">CrÃ©er le produit libre</button></div></div>}
      <div className="bg-white border border-slate-100 rounded-2xl p-4 md:p-5 shadow-premium space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <label><span className="sr-only">Filtrer par catÃ©gorie</span><select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-3"><option value="">Toutes les catÃ©gories</option>{categories.map((x) => <option key={x}>{x}</option>)}</select></label>
        </div>
        <div className="flex flex-wrap gap-2 items-center"><button onClick={() => setOrderOnly(!orderOnly)} className={`rounded-full px-4 py-2 text-sm flex items-center gap-1 ${orderOnly ? "bg-red-700 text-white" : "border border-red-200 text-red-700"}`}><Bell className="w-4 h-4" /> Ã€ commander</button><button onClick={reset} className="rounded-full px-4 py-2 text-sm border border-slate-200 flex items-center gap-1"><SlidersHorizontal className="w-4 h-4" /> RÃ©initialiser</button><label className="ml-auto flex items-center gap-2 text-sm"><input type="checkbox" checked={showPrices} onChange={(e) => setShowPrices(e.target.checked)} className="w-4 h-4 accent-[#0A192F]" /> Afficher les tarifs</label></div>
        {showPrices && <div className="text-sm text-right">Valeur totale du stock visible : <strong>{euro(total)}</strong></div>}
      </div>

      {filtered.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 p-10 text-center text-slate-500">{list.length === 0 ? "Aucun produit en stock. Ajoutez votre premiÃ¨re rÃ©fÃ©rence depuis le catalogue." : "Aucun produit ne correspond Ã  ces filtres."}</div> : Object.entries(grouped).map(([b, cats]) => {
        const brandKey = `b:${b}`; const closed = collapsed[brandKey];
        return <section key={b} className="space-y-3"><button onClick={() => setCollapsed({ ...collapsed, [brandKey]: !closed })} className="w-full flex items-center gap-2 text-left"><span className="p-1 rounded-full bg-slate-100">{closed ? <ChevronRight className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}</span><h2 className="font-serif text-2xl">{b}</h2></button>{!closed && Object.entries(cats).map(([c, items]) => { const key = `${b}:${c}`; const catClosed = collapsed[key]; return <div key={c} className="ml-0 md:ml-4"><button onClick={() => setCollapsed({ ...collapsed, [key]: !catClosed })} className="w-full flex items-center gap-2 text-left mb-2"><span>{catClosed ? <ChevronRight className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}</span><h3 className="text-sm font-semibold uppercase tracking-wide text-slate-600">{c} Â· {items.length}</h3></button>{!catClosed && <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">{items.sort((a, z) => (a.name || "").localeCompare(z.name || "", "fr")).map((item) => <StockLine key={item.id} item={item} showPrices={showPrices} onChanged={load} />)}</div>}</div>})}</section>;
      })}
      {showAdd && <AddProductDialog catalog={catalog} onClose={() => setShowAdd(false)} onAdded={load} />}
    </div>
  );
}
