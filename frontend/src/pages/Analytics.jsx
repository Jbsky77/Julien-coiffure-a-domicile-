...
export default function Analytics() {
  ...
  const [projectionYear, projectionMonth] = selectedMonth.split("-").map(Number);
  const projectionToday = new Date();
  const projectionDaysInMonth = new Date(projectionYear, projectionMonth, 0).getDate();
  const projectionSelectedIndex = projectionYear * 12 + projectionMonth;
  const projectionCurrentIndex = projectionToday.getFullYear() * 12 + projectionToday.getMonth() + 1;
  const projectionObservedDays = projectionSelectedIndex < projectionCurrentIndex ? projectionDaysInMonth : projectionSelectedIndex === projectionCurrentIndex ? Math.min(projectionToday.getDate(), projectionDaysInMonth) : 0;
  const projectionDaily = projectionObservedDays > 0 ? Number(d.total_ca || 0) / projectionObservedDays : 0;
  const projectionMonthTotal = projectionDaily * projectionDaysInMonth;
      ...
      <section className="bg-white border border-violet-100 rounded-2xl p-5 space-y-4 shadow-sm" data-testid="revenue-projection">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-2">Estimation sur {projectionObservedDays} jour{projectionObservedDays > 1 ? "s" : ""}, soit {money2(projectionDaily)} € par jour.</p>
          </div>
          <span className="text-[10px] uppercase tracking-wider text-violet-700 bg-violet-50 rounded-full px-3 py-1">Estimation</span>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {[
            ["Mois", projectionMonthTotal, `fin du mois (${projectionDaysInMonth} jours)`],
            ["Trimestre", projectionMonthTotal * 3, "3 mois au même rythme"],
            ["Année", projectionMonthTotal * 12, "12 mois au même rythme"],
          ].map(([label, value, detail]) => (
            <div key={label} className="rounded-2xl bg-gradient-to-br from-violet-50 to-white border border-violet-100 p-4">
              <div className="text-[10px] uppercase tracking-widest text-slate-500">{label}</div>
              <div className="font-serif text-2xl text-violet-700 mt-1">{money2(value)} €</div>
              <div className="text-[10px] text-slate-400 mt-1">{detail}</div>
            </div>
          ))}
        </div>
        <p className="text-[10px] text-slate-400">Projection indicative basée sur un rythme identique pendant toute la période.</p>
      </section>
